...
def list_users( income_id = Depends(Auth.wrapper), *, db: Session = Depends(db_session),  config: ListRequestSchema ) -> Any:    
    users_list = UsersService.create_users_list(db, config=config)
...
def update_user(
    ...
    *,
    ...
    AuthService.validate_admin_or_current_access(db, income_id=income_id, action_id=id)

    current_user = UsersService.validate_id(db, id=id)
    updated_user = UsersService.update_user(db, db_object=current_user, infos_object=new_infos)
